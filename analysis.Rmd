...
```{r}
# Segmentation analysis: Productivity Change by Sector & Working from Home
...
if (all(c("Childcare_Responsibilities", "Work_From_Home", "Productivity_Change") %in% colnames(data))) {
  ...
  # Create a graph to visualize the analysis
   # Create a graph to visualize the analysis
  ggplot(childcare_analysis, aes(x = as.factor(Childcare_Responsibilities), y = Avg_Productivity, fill = as.factor(Work_From_Home))) +
    ...
    geom_text(aes(label = round(Avg_Productivity, 3)), 
              ...
              size = 3) +
    labs(
      title = "Impact of Childcare Responsibilities on Productivity by Work From Home Status",
      x = "Childcare Responsibilities (0 = No, 1 = Yes)",
      y = "Average Productivity Change",
      fill = "Work From Home (0 = No, 1 = Yes)"
    ) +
    coord_cartesian(ylim = c(0.45, 0.55)) +  # Display only from 0.45 to 0.55
    theme_minimal() +
...
if (all(c("Stress_Level", "Work_From_Home", "Productivity_Change") %in% colnames(data))) {
  ...
  ggplot(stress_analysis, aes(x = as.factor(Stress_Level), y = Avg_Productivity, fill = as.factor(Work_From_Home))) +
    ...
    geom_text(aes(label = round(Avg_Productivity, 3)), 
              ...
              size = 3) +
    labs(
      title = "Impact of Stress Level on Productivity by Work From Home Status",
...
if (all(c("Childcare_Responsibilities", "Commuting_Changes", "Health_Issue", "Technology_Adaptation") %in% colnames(data))) {
  ...
  ggplot(technology_adaptation_analysis, aes(
    ...
    labs(
      title = "Impact of Childcare, Commuting Changes, and Health Issues on Technology Adaptation",
      x = "Childcare Responsibilities (0 = No, 1 = Yes)",
...
#### Question 9: Which sectors have experienced the highest and lowest average productivity changes?
...
#### Question 10: Is there a significant difference in productivity change between employees who experienced a salary cut and those with stable salaries?
```{r}
# For Salary Changes vs Productivity Change
t_test_result <- t.test(salary_cut_group, stable_salary_group)
cat("T-test Results: t-value =", t_test_result$statistic, "p-value =", t_test_result$p.value, "\n")
```
#### Question 11: How does the combination of sector and work-from-home status affect average productivity change?
```{r}
# Interaction analysis between Sector and Work_From_Home
ggplot(segmentation_analysis, aes(x = Sector, y = Avg_Productivity, fill = as.factor(Work_From_Home))) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.8) +
  labs(
    title = "Productivity Change by Sector and Work From Home",
    x = "Sector",
    y = "Average Productivity Change",
    fill = "Work From Home (0 = No, 1 = Yes)"
  ) +
...
#### Question 12: How does productivity change vary across different sectors?
```{r}
# Example of a final plot - boxplot comparing productivity changes by sector
library(ggplot2)
ggplot(data, aes(x = Sector, y = Productivity_Change, fill = Sector)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Impact of Sector on Productivity Changes", 
       x = "Sector", y = "Productivity Change")
```

#### Question 13: What is the distribution of stress levels for employees who work from home versus those who don't?
```{r}
